...
"${COMPOSE_CMD[@]}" exec -T cassandra cqlsh -e "CREATE KEYSPACE IF NOT EXISTS logistica WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1}; USE logistica; CREATE TABLE IF NOT EXISTS vehicle_latest_state (ship_id text PRIMARY KEY, event_ts timestamp, route_id text, origin_port text, dest_port text, warehouse text, lat double, lon double, speed_kn double, heading double, stock_on_hand int, reorder_point int, updated_at timestamp); TRUNCATE logistica.vehicle_latest_state;"

echo "Extrayendo ultimo estado por vehiculo desde Hive..."
"${COMPOSE_CMD[@]}" exec -T spark spark-submit /home/jovyan/jobs/spark/04_export_latest_vehicle_state.py > "${TMP_TSV}"
